...
DB_CONFIG = {
    'user': 'sonofwau',
    'password': 'x=u~vdFKM&t&',
    'host': 'localhost', # or your MySQL server IP/hostname
    'database': 'sonofwau$msl_app_db', # Create this database in MySQL
...
def get_tasks_logic(filter_property="Filter_UI"):
    ...
    for task in tasks:
        ...
        if isinstance(due_date, datetime):
            ...
            if time_span.total_seconds() < 0: time_val = "---Overdue---"
            elif 0 <= time_span.total_days() <= 1: time_val = "---Today---"
            elif 1 < time_span.total_days() <= 7: time_val = "---This Week---"
            elif 7 < time_span.total_days() <= 14: time_val = "---Next Week---"
